...
function botInit() {
    bot.getMe().then(function (me) {
        ...
        setInterval(checkTxQuery, txInterval * 7);
        sendMessageByBot(devChatId, msg, 0, false);
        getChatAdmins();
        sendTxByInterval().then(() => {
        }); // start Tx process timer
        checkRetweetTimer();
...
function checkTxQuery() {
    if (globalTxQuery.length) {
        const obj = globalTxQuery[0];
...
function checkGiveUser(aCallbackQuery, aItem, aSourceItem) {
    ...
    if (aItem) {
        if (aItem.wallet_address) {
            ...
            let messageText ='You have not set WALLET address in ' + globalBotUserName;
...
function parseMessage(msg, aAdmin) {
    ...
    if (chatType !== 'private') {
        ...
        if ((messageText.indexOf(`/${chatCmdMod}tip`) === 0)) {
            console.log('tip user ', messageChatId, messageText);
                doTip(msg);
            return;
            // Call giveaway function
        } else if ((messageText.indexOf(`/${chatCmdMod}giveaway`) === 0)) {
            console.log('giveaway value ', messageChatId, messageText);
            doGiveaway(msg);
            if (toDeleteMessage) {
                setTimeout(deleteBotMessage, deleteMessageInterval * 2, msg);
            }
            ...
            console.log('get active token ', messageChatId, messageText);
            doCheckToken(msg);
            if (toDeleteMessage) {
                setTimeout(deleteBotMessage, deleteMessageInterval * 2, msg);
            }
            ...
            console.log('rain value ', messageChatId, messageText);
            doRain(msg);
            if (toDeleteMessage) {
                setTimeout(deleteBotMessage, deleteMessageInterval * 2, msg);
            }
            ...
            console.log('twitter rain ', messageChatId, messageText);
            doTwitterRain(msg);
            // if (toDeleteMessage) {
            //     setTimeout(deleteBotMessage, deleteMessageInterval * 2, msg);
            // }
            ...
            console.log('start tweet monitoring ', messageChatId, messageText);
            startTweet(msg);
            // if (toDeleteMessage) {
            //     setTimeout(deleteBotMessage, deleteMessageInterval * 2, msg);
            // }
            ...
            console.log('giveaway cyclic ', messageChatId, messageText);
            doGiveawayAuto(msg);
            if (toDeleteMessage) {
                setTimeout(deleteBotMessage, deleteMessageInterval * 2, msg);
            }
            ...
            console.log('stop giveaway cyclic ', messageChatId, messageText);
            doGiveawayStop(msg);
            if (toDeleteMessage) {
                setTimeout(deleteBotMessage, deleteMessageInterval * 2, msg);
            }
    ...
    if ((messageText.indexOf('/register') === 0) || (messageText === catchPhrases.mainKeyboard[0])) {
        console.log('register wallet ', messageChatId, messageText);
    ...
    // default tip value
    else if ((messageText === '/defval') || (messageText === catchPhrases.settingsKeyboard[6])) {
        getDefaultTip(msg);
    }
    ...
    // default tip value
        ...
        // console.log('get twitter user info ', messageChatId, messageText);
        setTwitUser(msg);
        // View user's balance
    ...
    // default
        ...
        if (getAdminRights(messageUserName)) { // admin commands
            ...
            if ((messageText.indexOf('/message') === 0) ) {
                ...
                console.log('hostip command');
                sendHostIpToChat(messageChatId, messageId, messageChatId);
            }
...
function dbRemoveChat(chatId) {
    console.log('dbRemoveChat: ', chatId);
    mongoChatsTable.deleteOne({chat_id: chatId})
        .catch(e => {
            console.log('mongo error: ' + e);
        });
}
            ...
            doc.forEach(function (item) {
                bot.getChatMember(item.chat_id, aUserId)
                    ...
                    .catch(e => {
                        ...
                        if ((eStr.indexOf('was kicked') !== -1) || (eStr.indexOf('chat not found') !== -1)) { // blocked by user
                            dbRemoveChat(item.chat_id);
                        }
                    });
            });
        }
...
function generateSettingsKeyboard(aUserId) {
    ...
    if (isAdmin) {
        ...
        row.push(catchPhrases.settingsKeyboard[6]); // Defaul values
    }
    kbd.push(row);
    row = [];
...
function checkTipUser(aMsg, aItem, aObj) { // item - dest User
    ...
    if (aItem) {
        if (aItem.wallet_address) {
            ...
            let messageText = 'User have not set WALLET address in ' + globalBotUserName;
...
function getTokenName(aDbItem) {
    if (aDbItem) {
        if (aDbItem.def_token) {
            return aDbItem.def_token;
        }
    }
    return Object.keys(dbconfig.tokens)[0];
}
...
function checkTip(aMsg, aItem) { // item = Source User
    ...
    if (aItem) {
        ...
        if (tipAmount < 0) {
            if (aItem[`${tokenName}_tip_value`]) {
                tipAmount = aItem[`${tokenName}_tip_value`];
            } else {
...
function checkTwRain(aMsg, aItem) {
    ...
    if (aItem)
    {
        ...
        let minValue = dbconfig.minValues[tokenName];
        if (rainValue === -1) {
            rainValue = aItem[`${tokenName}_rain_value`] || (minValue * 10);
        }
...
function checkGiveaway(aMsg, aItem) {
    ...
    if (aItem)
    {
        ...
        if (giveValue === -1) {
            if (aItem[`${tokenName}_tip_value`]) {
                giveValue = aItem[`${tokenName}_tip_value`];
            } else {
...
function doTwitterRain(aMsg) {
    ...
    if (strs.length > 1) { // if param  exists in message
        if (strs.length === 2) {
            aMsg.text = `${strs[0]} -1 ${strs[1]}`;
        }
        let userId = 0;
        if (aMsg.chat.type !== 'channel') {
            userId = aMsg.from.id;
        }
        if (userId) {        // check user registered
...
function getDefTokenName(aItem) {
    let val;
    if (aItem) {
        val = aItem.def_token;
    }
...
function getDefaultTip(aMsg) {
    ...
    mongoUsersTable.findOne({user_id: id}, {}, function(err, item) {
        ...
        let defToken = getDefTokenName(item);
        if (item) {
            let valTip = item[`${defToken}_tip_value`] || dbconfig.minValues[defToken];
            let valRain = item[`${defToken}_rain_value`] || dbconfig.minValues[defToken];
            messageText = `For <code>${defToken}</code> defaul values are:
- <code>tip/give</code> = ${valTip} 
- <code>rain</code> = ${valRain} 
Enter new values in format: <code>tip_value rain_value</code>`;
            }
        sendMessageByBot(chatId, messageText, userId, false);
        chatStateMap.set(userId, ChatStateEnum.WaitDefValueState);
        userTokenMap.set(userId, defToken);
    })
}
...
async function setNewWallet(aMsg) {
    let chatId = aMsg.chat.id;
    let userId = aMsg.from.id;
    let text = aMsg.text.trim();
    ...
    if (bnbClient.checkAddress(text, 'bnb')) {
        ...
        const accnt = await bnbClient.getAccount(text);
        // console.log(accnt);
        if (accnt.result.flags) {
            let messageText = catchPhrases.privateDialogMessages[38];
            sendMessageByBot(chatId, messageText, userId, false);
            chatStateMap.delete(userId);
            return;
        }
        ...
        .then((data) => {
            mongoUsersTable.findOne({twit_user_id: data.id_str}, (err, item) => {
                if (err) {
                    ...
                    if ((item) && (userId.toString() !== item.user_id)) {
...
function setDefaultTip(aMsg) {
    ...
    let messageText = aMsg.text;
    let strs = messageText.split(/(\s+)/).filter( e => e.trim().length > 0);
    let valTip = Number(strs[0].replace(',', '.'));
    let defToken = userTokenMap.get(userId);
    let valRain = 0;
    let tipField = `${defToken}_tip_value`;
    let rainField = `${defToken}_rain_value`;
    if (strs.length > 1) {
        valRain = Number(strs[1].replace(',', '.'));
    } else {
        valRain = dbconfig.minValues[defToken];
    }
    if (isNaN(valTip) || isNaN(valRain)) {
        let messageText = catchPhrases.privateDialogMessages[1];
        sendMessageByBot(chatId, messageText, userId, false);
    } else {
        mongoUsersTable.updateOne({user_id: userId.toString()},
            {$set: {[tipField]: valTip, [rainField]: valRain}})
            .then(() => {
                let messageText = `${valTip} ${defToken} ${catchPhrases.privateDialogMessages[2]}\n`;
                messageText += `and ${valRain} ${defToken} ${catchPhrases.privateDialogMessages[37]}`;
                sendMessageByBot(chatId, messageText, userId,  false);
            })
            .catch(e => {
                console.log('mongo error: ' + e);
...
function getAdminRights(aUserName) {
    let userName = aUserName.toUpperCase().trim();
    if (userName) {
        return dbconfig.admins.includes(userName);
...
function adminBotRestart(aMsg) {
    // todo save query before restart
    // if (globalTxQuery.length === 0) {
        if (restartTimeOut) {
...
function addTxToQuery(aItem, aOutputs, aDestItems, aValue, aMsg, aCallBack) {
    ...
    console.log('Tx Source: ', aItem.wallet_address);
    console.log('Tx Value: ', aValue);
    let src = cloneObj(aItem);
    let msg = aMsg;
    let cbk = cloneObj(aCallBack);
    let memo = 'memo';
    switch (aCallBack.name) {
        case 'cbkFinalizeRain':
            memo = 'TWT tip.bot Rain';
            break;
        case 'cbkFinalizeTip':
            memo = 'TWT tip.bot Tip';
            break;
        case 'cbkFinalizeGive':
            memo = 'TWT tip.bot Giveaway';
            break;
    }
...
async function sendTxByInterval() {
    if (globalTxQuery.length) {
        ...
        let aOutputs = JSON.parse(JSON.stringify(txObj.outputs));
        let aCallBack = txObj.fnCallBack;
        const memo = txObj.memo;
        ...
        if (isDebugMode) {
            ...
            setTimeout(sendTxByInterval, txInterval);
            bnbClient.multiSend(aItem.wallet_address, aOutputs, memo)
                .then(res => {
                ...
                .catch(err => {
                    ...
                    if (err.toString().indexOf('insufficient') !== -1) {
                        sendReplyMessageByBot(aMsg.chat.id, 'Tx error: insufficient fund (check if you have BNB for fee)',
                            aMsg.message_id, true);
                    } else if (err.toString().indexOf('non-empty memo') !== -1) {
                        txObj.memo = 'memo';
                        globalTxQuery.push(txObj);
                        // sendReplyMessageByBot(aMsg.chat.id, 'Tx error: receiver requires non-empty memo',
                        //     aMsg.message_id, true);
                    } else if (err.toString().indexOf('only digits') !== -1) {
                        txObj.memo = '123';
                        globalTxQuery.push(txObj);
                        // sendReplyMessageByBot(aMsg.chat.id, 'Tx error: receiver requires the memo contains only digits',
                        //     aMsg.message_id, true);
                    } else {
                        // sendReplyMessageByBot(aMsg.chat.id, err.toString(), aMsg.message_id, true);
                    }
                });
        }
    } else {
...
function sendHostIpToChat(aMessageChatId, aMsgId, aUserId) {
    console.log ('request Host IP');
    Exec('curl http://ipecho.net/plain', function(err, stdout) {
        if (err) {
            console.log ('request Host IP - ERROR');
            sendMessageByBot(aMessageChatId, catchPhrases.debugCommandMessages[4], aUserId);
            return;
        }
        if (!isEmpty(stdout)) {
            console.log ('!isEmpty(stdout)');
            sendMessageByBot(aMessageChatId, catchPhrases.debugCommandMessages[3] + stdout, aUserId);
        } else {
            console.log ('isEmpty(stdout)');
            sendMessageByBot(aMessageChatId, catchPhrases.debugCommandMessages[5], aUserId);
        }
    });
}
